...
public class SideCarPasteAction implements IAction {
	...
	public static IAction getInstance(IAction action) {
		if (instance == null) {
			instance = new SideCarPasteAction((ClipboardOperationAction) action);
			PasteHandler.setPasteAction(instance);
		}
		return instance;
	}
	...
	private SideCarPasteAction(ClipboardOperationAction c) {
		clip = c;
	}
	...
	public void run() {
		actionPerformed(); // what are the contents right before we paste?
		clip.run();
	}
